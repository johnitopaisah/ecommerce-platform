          ...
          {!order.billing_status && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
              <p className="text-sm text-amber-800 mb-3">
                Payment not confirmed yet. Mark manually if received outside Stripe.
              </p>
              <Button size="sm" variant="primary" onClick={handleMarkPaid} isLoading={markingPaid} className="w-full">
            ...
            <div className="flex gap-2 flex-wrap">
              {nextStatuses.map((s) => (
                <Button key={s} size="sm"
                  ...
                  onClick={() => handleUpdate(s)} isLoading={updating} className="capitalize">
                  Mark as {s}
                </Button>
              ))}
            </div>
