  ...
  const onProfileSubmit = async (data: ProfileData) => {
    try {
      ...
      profileForm.setError("root", {
        message: e?.response?.data?.detail || "Update failed.",
      });
    }
  };
  ...
  const onPasswordSubmit = async (data: PasswordData) => {
    try {
      ...
      passwordForm.setError("root", {
        message: e?.response?.data?.detail || "Password change failed.",
      });
    }
  };
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <form onSubmit={profileForm.handleSubmit(onProfileSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Input id="first_name" label="First name" {...profileForm.register("first_name")}
              error={profileForm.formState.errors.first_name?.message} />
            <Input id="last_name" label="Last name" {...profileForm.register("last_name")}
              error={profileForm.formState.errors.last_name?.message} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label className="text-sm font-medium text-gray-500">Email</label>
              <p className="text-sm text-gray-400 bg-gray-50 rounded-lg px-3 py-2 border border-gray-200">{user?.email}</p>
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-sm font-medium text-gray-500">Username</label>
              <p className="text-sm text-gray-400 bg-gray-50 rounded-lg px-3 py-2 border border-gray-200">{user?.user_name}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <Input id="country" label="Country code" placeholder="e.g. GB" maxLength={2}
              {...profileForm.register("country")} error={profileForm.formState.errors.country?.message} />
          </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <form onSubmit={passwordForm.handleSubmit(onPasswordSubmit)} className="space-y-4">
          <Input id="old_password" label="Current password" type="password"
            {...passwordForm.register("old_password")} error={passwordForm.formState.errors.old_password?.message} />
          <Input id="new_password" label="New password" type="password"
            {...passwordForm.register("new_password")} error={passwordForm.formState.errors.new_password?.message} />
          <Input id="new_password2" label="Confirm new password" type="password"
            {...passwordForm.register("new_password2")} error={passwordForm.formState.errors.new_password2?.message} />
